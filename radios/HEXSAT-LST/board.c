...
void board_init(void) {
    // RF_PSU_SHDN setup
    P0SEL &= ~(0<<0);  // GPIO not peripheral
    P0DIR |= 1<<0;  // Output not input
    //P0_0 = 1; // Enable RF PSU

	// LED0 setup - just turn it on (green)
	...
	// RF_ENABLE setup
	//P2SEL &= ~(1<<2);  // GPIO not peripheral
	//P2DIR |= 1<<2;  // Output not input
	//P2_2 = 0; // RF_ENABLE in low
